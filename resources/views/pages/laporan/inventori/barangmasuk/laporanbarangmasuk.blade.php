                ...
                <div class="filter-card">
                    ...
                    <div class="filter-section d-flex justify-content-between align-items-center gap-4">
                        ...
                        <div class="filter-right d-flex align-items-center gap-2">
                            <button type="button" id="btn-filter" class="btn btn-primary btn-sm" style="margin-right: 10px"
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#laporan-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('laporanbarangmasuk.data') }}',
                    data: function(d) {
        ...
        function showDetail(id) {
            $.ajax({
                url: '{{ route('laporanbarangmasuk.detail', ':id') }}'.replace(':id', id),
                type: 'GET',
                success: function(response) {
                    ...
                    <div class="row">
                        ...
                        data.detail_barang_masuk.forEach(function(detail) {
                            html += `
                        <tr>
                            <td>${detail.barang ? detail.barang.nama_barang : '-'}</td>
                            <td class="text-center">${detail.jumlah}</td>
                            <td>${detail.satuan ? detail.satuan.nama : '-'}</td>
                            <td class="text-right">Rp ${new Intl.NumberFormat('id-ID').format(detail.harga_beli)}</td>
                        ...
                        html += `
                                    ...
                                    <tfoot>
                                        <tr class="font-weight-bold">
                                            ...
                                            <td class="text-right">Rp ${new Intl.NumberFormat('id-ID').format(data.detail_barang_masuk.reduce((sum, detail) => sum + parseFloat(detail.total), 0))}</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
