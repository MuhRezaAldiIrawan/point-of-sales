        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="detailModalLabel">Detail Item Barang Masuk</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#laporan-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('laporandetailbarangmasuk.data') }}',
                    data: function(d) {
        ...
        function showDetail(id) {
            $.ajax({
                url: '{{ route('laporandetailbarangmasuk.detail', ':id') }}'.replace(':id', id),
                type: 'GET',
                        ...
                        <div class="col-md-6">
                            <table class="table table-sm">
                                <tr>
                                    ...
                                    <td>${data.barang_masuk && data.barang_masuk.tanggal_masuk ? new Date(data.barang_masuk.tanggal_masuk).toLocaleDateString('id-ID') : '-'}</td>
                                </tr>
                                <tr>
                                    <th>No. Reff:</th>
                                    <td>${data.barang_masuk ? (data.barang_masuk.no_reff || '-') : '-'}</td>
                                </tr>
                                <tr>
                                    <th>Jenis Stok:</th>
                                    <td>${data.barang_masuk && data.barang_masuk.jenis_stok ? data.barang_masuk.jenis_stok.nama : '-'}</td>
                                </tr>
                                <tr>
                                    <th>Status:</th>
                                    <td>${data.barang_masuk && data.barang_masuk.status === 'cancelled' ? '<span class="badge badge-danger">Cancelled</span>' : '<span class="badge badge-success">Success</span>'}</td>
                                </tr>
                                <tr>
                                    <th>Alasan Cancel:</th>
                                    <td>${data.barang_masuk ? (data.barang_masuk.cancel_reason || '-') : '-'}</td>
                                </tr>
                                <tr>
                                    <th>Nama Barang:</th>
                                ...
                                <tr>
                                    <th>Jumlah:</th>
                                    <td>${data.jumlah} ${data.satuan ? data.satuan.nama : ''}</td>
                                </tr>
                            </table>
                        </div>
                        <div class="col-md-6">
                            <table class="table table-sm">
                                <tr>
                                    ...
                                    <td class="text-right">Rp ${data.harga_beli ? new Intl.NumberFormat('id-ID').format(data.harga_beli) : '0'}</td>
                                </tr>
                                <tr>
                                    <th>Total:</th>
                                    <td class="text-right">Rp ${data.total ? new Intl.NumberFormat('id-ID').format(data.total) : '0'}</td>
                                </tr>
                                <tr>
                                    <th>Keterangan:</th>
                                ...
                                <tr>
                                    <th>Dibuat Oleh:</th>
                                    <td>${data.created_by ? data.created_by.name : (data.barang_masuk && data.barang_masuk.created_by ? data.barang_masuk.created_by.name : '-')}</td>
                                </tr>
                                <tr>
                                    <th>Dicancel Oleh:</th>
                                    <td>${data.barang_masuk && data.barang_masuk.cancelled_by ? data.barang_masuk.cancelled_by.name : '-'}</td>
                                </tr>
                                <tr>
                                    <th>Dibuat Pada:</th>
