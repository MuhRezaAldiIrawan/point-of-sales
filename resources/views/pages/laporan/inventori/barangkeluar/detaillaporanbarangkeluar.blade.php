        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="detailModalLabel">Detail Item Barang Keluar</h5>
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#laporan-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('laporandetailbarangkeluar.data') }}',
                    data: function(d) {
        ...
        function showDetail(id) {
            $.ajax({
                url: '{{ route('laporandetailbarangkeluar.detail', ':id') }}'.replace(':id', id),
                type: 'GET',
                success: function(response) {
                    if (response.success) {
                        const data = response.data;

                        // Check if barang_keluar relationship exists
                        if (!data.barang_keluar) {
                            toastr.error('Data barang keluar tidak ditemukan');
                            return;
                        }
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <table class="table table-sm">
                                <tr>
                                    <th width="40%">Tanggal Keluar:</th>
                                    <td>${data.barang_keluar && data.barang_keluar.tanggal_keluar ? new Date(data.barang_keluar.tanggal_keluar).toLocaleDateString('id-ID') : '-'}</td>
                                </tr>
                                <tr>
                                    <th>No. Reff:</th>
                                    <td>${data.barang_keluar ? (data.barang_keluar.no_reff || '-') : '-'}</td>
                                </tr>
                                <tr>
                                    <th>Jenis Stok:</th>
                        ...
                        <div class="col-md-6">
                            <table class="table table-sm">
                                <tr>
                                    ...
                                    <td class="text-right">Rp ${data.harga_jual ? new Intl.NumberFormat('id-ID').format(data.harga_jual) : '0'}</td>
                                </tr>
                                <tr>
                                    <th>Total:</th>
                        ...
                        toastr.error(response.message || 'Gagal memuat detail');
                    }
                },
                error: function(xhr, status, error) {
                    toastr.error('Terjadi kesalahan saat memuat detail: ' + error);
                }
            });
        }
    </script>
